...
def configure_device():
    ...
    if gpus:
        try:
            ...
            logger.info("GPU(s) available: %s – training on CUDA.", [g.name for g in gpus])
            return "GPU", len(gpus)
        except RuntimeError as exc:  # memory growth must be set before GPUs are initialised
            logger.warning("Could not configure GPU memory growth (%s); continuing.", exc)
            return "GPU", len(gpus)
...
def build_model(num_classes: int, weights: str = "imagenet",
                backbone: str = "mobilenetv2", img_size: int = 160):
...
def compute_metrics(y_true, y_pred, class_names):
    ...
    weighted_f1 = (
        sum(per_class[c]["f1"] * per_class[c]["support"] for c in class_names) / support_total
        if support_total else 0.0
    )
    accuracy = correct / support_total if support_total else 0.0
    overall = {"accuracy": round(accuracy, 4), "macro_f1": round(macro_f1, 4),
...
# Training
# ---------------------------------------------------------------------------
def train_dataset(name: str, epochs: int = 15, batch_size: int = 32, image_size: int = 160,
                  backbone: str = "mobilenetv2", weights: str = "imagenet",
                  finetune_epochs: int = 10, out_name: str = None,
                  device: str = "CPU"):
    ...
    metrics = {
        "dataset": name, "out_name": out_name, "backbone": backbone,
        "image_size": image_size, "device": device,
        "num_classes": num_classes, "total_params": int(model.count_params()),
        "accuracy": overall["accuracy"], "macro_f1": overall["macro_f1"],
    ...
    _write_report_md(name, out_name, backbone, image_size, device, overall, per_class,
                     cm, save_dir / "benchmark_report.md", png_ok)
...
def _write_report_md(name, out_name, backbone, image_size, device, overall, per_class,
                     cm, out_path: pathlib.Path, png_ok: bool):
    lines = [
    ...
    for c, m in per_class.items():
        lines.append(f"| {c} | {m['precision']:.3f} | {m['recall']:.3f} | {m['f1']:.3f} | {m['support']} |")
    lines += ["", "## Confusion matrix", ""]
    if png_ok:
        lines.append(f"![confusion matrix]({out_path.parent.name}/confusion_matrix.png)")
        lines.append("")
    header = " | ".join([""] + list(per_class.keys()) + [""])
    lines.append(header)
    for c, m in per_class.items():
        row = [c] + [str(cm[i, list(per_class).index(c)]) for i in range(len(per_class))]
...
# ---------------------------------------------------------------------------
def train_both(name: str, epochs: int = 15, batch_size: int = 32, image_size: int = 224,
               weights: str = "imagenet", finetune_epochs: int = 10, device: str = "CPU"):
    ...
    for backbone in ("mobilenetv2", "efficientnetb0"):
        ...
        metrics = train_dataset(
            name, epochs=epochs, batch_size=batch_size, image_size=image_size,
            backbone=backbone, weights=weights, finetune_epochs=finetune_epochs,
            out_name=out_name, device=device,
        )
        results[backbone] = metrics

    best = max(results, key=lambda b: results[b]["macro_f1"])
    logger.info("Best backbone by validation macro-F1: %s (%.4f) vs %s (%.4f)",
                best, results[best]["macro_f1"],
                [b for b in results if b != best][0],
                results[[b for b in results if b != best][0]]["macro_f1"])

    src = models_root / f"{name}_{best}"
    for fname in ("model.keras", "class_names.txt", "metrics.json",
                  "benchmark_report.md", "confusion_matrix.png"):
        s = src / fname
        if s.exists():
...
def _write_skin_summary(name, live_dir, results, best):
    ...
    for b, m in results.items():
        lines.append(f"| {b} | {m['accuracy']:.4f} | {m['macro_f1']:.4f} | "
                     f"{m['weighted_f1']:.4f} | {m['total_params']:,} | {m['device']} |")
    lines.append("")
    lines.append(f"The winner was copied to `models/{name}/` and is served by "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-size", type=int, default=160, help="Input image size (default 160)")
    parser.add_argument(
        "--backbone", choices=["mobilenetv2", "efficientnetb0"], default="mobilenetv2",
    ...
    parser.add_argument(
        "--finetune-epochs", type=int, default=10, help="Phase-2 fine-tune epochs (default 10)"
    )
    parser.add_argument(
    ...
    if args.train_both:
        train_both(
            ...
            image_size=args.image_size, weights=args.weights,
            finetune_epochs=args.finetune_epochs, device=device,
        )
    else:
        train_dataset(
            args.dataset, epochs=args.epochs, batch_size=args.batch_size,
            image_size=args.image_size, backbone=args.backbone, weights=args.weights,
            finetune_epochs=args.finetune_epochs, out_name=args.out_name, device=device,
        )
